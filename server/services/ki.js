...
const WRITING_RULES = [
  ...
  'WICHTIG — keine erfundenen Tatsachen: Der Beschluss darf nur Tatsachen feststellen oder voraussetzen, die der Nutzer genannt oder bestätigt hat. Erfinde keine Paragrafen-Nummern des Gesellschaftsvertrags, keine Anteils-Stückelung, keine Besicherungsabreden, keine Begründungen (z.B. "wegen gestiegener Verantwortung") und keine eingeholten Vergleichsangebote. Fehlt eine für die Bestimmtheit nötige Tatsache, setze einen Platzhalter in eckigen Klammern (z.B. "[Nummer des Geschäftsanteils]", "[Stichtag der BWA]") und weise in "reply" darauf hin.',
...
function buildReconcilerSystem(ctx) {
  return [
    ...
    'Formal: keine erfundenen Tatsachen (fehlende Angaben als Platzhalter in eckigen Klammern + Hinweis in reply), keine Anlagen-Verweise. "content" beginnt direkt mit "1.", nummerierte Punkte mit Leerzeile dazwischen, kein Markdown. Der formale Rahmen des Beschlusses wird automatisch erzeugt — du lieferst nur den variablen Teil.',
    NORM_LIB,
    ctx,
  ].join('\n')
}
...
export async function runBeschlussChat({
  ...
  draft.reply = String(draft.reply ?? '')
  ...
  try {
    ...
    return {
      reply: String(final.reply ?? draft.reply),
      writeContent: true,
      content,
